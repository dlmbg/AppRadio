...
class pembayaran extends CI_Controller {
   ...
   public function edit($id_param)
   {
		if($this->session->userdata("logged_in")!=""  && $this->session->userdata("level")=="admin")
		{
			...
			$get = $this->db->query("select * from dlmbg_pembayaran a left join (SELECT `id_transaksi_pemasangan`, alamat_pelanggan, tanggal, `nama_pelanggan`, `promo`, `st`, `prime_time`, `regular_time`, `durasi_iklan`, `volume_tayang`, `jumlah_biaya`, `uang_muka` FROM (`dlmbg_transaksi_pemasangan`) JOIN `dlmbg_pelanggan` ON `dlmbg_pelanggan`.`kode_pelanggan`=`dlmbg_transaksi_pemasangan`.`kode_pelanggan` JOIN `dlmbg_tarif_iklan` ON `dlmbg_tarif_iklan`.`id_tarif_iklan`=`dlmbg_transaksi_pemasangan`.`id_tarif_iklan`) b on a.id_transaksi_pemasangan=b.id_transaksi_pemasangan where a.id_pembayaran='".$id_param."' ")->row();
